        ...
        while True:
            ...
            if self.stats['port']:
                ...
                if self.manager.link_free_bw and self.shortest_paths:
                    ...
                    self.manager.update_tm(self.alg_name)
                self.show_stat('link')
...
#------------------------------------------------------------------------------------
    ...
    def ip_to_dpid(self, ip):
        return self.awareness.ip_to_dpid(ip)
    ...
    def flow_stats_reply_handler(self, ev):
        ...
        tm_delta = defaultdict(int)   # {(src_dpid, dst_dpid): bytes}
        ...
        for stat in body:
            ...
            key_full = (dpid, src_ip, dst_ip)
            prev = self.last_flow_bytes.get(key_full, 0)
            delta = bytes_now - prev
            self.last_flow_bytes[key_full] = bytes_now

            if delta <= 0:
                continue

            src_dpid = self.ip_to_dpid(src_ip)
            dst_dpid = self.ip_to_dpid(dst_ip)
            tm_delta[(src_dpid, dst_dpid)] += delta

        self.manager.tm_bytes = tm_delta
