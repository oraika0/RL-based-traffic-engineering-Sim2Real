...
def generate_traffic(data_file: str, out_dir: str, duration: int = 80000):
    for i in range(24):
        ...
        # (kai)原始數據中有自己傳給自己的流量，這裡將其設為0.0 
        #     <IntraTM ASID="20965">
        # <src id="12">
        #   <dst id="12">294730.8356</dst>
        for src in range(1,24):
            for dst in range(1,24):
                if src == dst and tm[src][dst] != 0.0:
                    tm[src][dst] = 0.0
        
        # (kai) port format: xx0yy.format(src,dst)(4(src 1 digit) or 5 digits(src 2 digits))
        for src in range(1,24):
            ...
            for dst in range(1,24):
                ...
                # (kai) original Geant bandwidth:10Gbps, 2.5Gbps, 150Mbps (too large for mininet)
