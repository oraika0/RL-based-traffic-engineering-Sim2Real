...
class Manager(app_manager.RyuApp):
    ...
    def update_tm(self, alg_name):
        """
        tm_delta_dict: {(src_dpid, dst_dpid): bytes_in_last_period}
        """
        if not self.tm_bytes:
            return
        ...
        snapshot = {
            ...
            "tm_bytes": {f"{k[0]}-{k[1]}": v for k, v in self.tm_bytes.items()}
        }
        with open(f"../results/{alg_name}/tm_snapshot.json", "w") as f:
