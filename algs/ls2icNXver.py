...
class ls2icNXver(nn.Module):
    def __init__(self, args):
        super(ls2icNXver, self).__init__()
        ...
        self.input_attention = MultiheadAttention(
            args.num_link_fea_NXver, args.num_link_fea_NXver, args.num_link_fea_NXver, self.hidden_dim, 4
        )
        self.layer_norm = nn.LayerNorm(args.num_link_fea_NXver)
        
        self.input_linear = nn.Linear(args.num_link * args.num_link_fea_NXver, self.hidden_dim)
...
class VAE(nn.Module):
    def __init__(self, args):
        ...
        input_dim = args.num_link * args.num_link_fea_NXver
...
class ls2icNXver_agent():
    def __init__(self, args):
        self.num_agents = args.num_agents
        self.action_dim = args.action_dim
        self.seq_dim = args.num_link
        self.num_link_fea_NXver = args.num_link_fea_NXver
        ...
        self.actor = ls2icNXver(args).to(self.device)
    ...
    def get_action(self, state, epsilon, **info):
        ...
        # state 可能是 numpy array，也可能是 list（保險處理）
        if isinstance(state, list):
            state = state[0]

        # 轉成 numpy array（確保連續記憶體）
        state = np.asarray(state, dtype=np.float32)

        # numpy -> torch（比 torch.tensor(list) 正確且快）
        state_tensor = torch.from_numpy(state).to(self.device)

        # 補 batch 維度：期望 (bs, num_agents, num_links, F)
